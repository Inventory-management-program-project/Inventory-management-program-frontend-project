...
const MobileProductDetail = () => {
        ...
        const fetchProductDetails = async () => {
            try {
                ...
                const [response1, response2, response3] = await Promise.all([
                    api.get(`/traders/stock/gcode-data/${gcode}`),
                    api.get(`/traders/goods/${gcode}`),
                    api.get(`/traders/movement/${gcode}`)
                ]);
    ...
    const updateLocation = async () => {
        try {
            ...
            const response = await api.put('/traders/stock/mobile-update-location', null, {
                params: {
                    gcode,
                    branchId,
